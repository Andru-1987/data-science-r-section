...
dependency <- c("tidyverse","DT","plotly","corrplot")
...
database_path <- here("week-1","clase_2","database","iris_database.db")
...
database_path <- here("week-1","clase_2","database","iris_database_tidyverse.db")
con <- dbConnect(SQLite(), database_path)
dbWriteTable(con, "iris_clean", iris_clean, overwrite = TRUE) 
...
local_storage <- here("week-1","clase_2", "images", "histograma_petalo.png")
...
### **Conclusión:**
- **Hipótesis nula (H₀)**: "La media de `Sepal_Length` en *versicolor* es 6 cm".  
- **Hipótesis alternativa (H₁)**: "La media no es 6 cm".  

**Resultado**:  
No podemos rechazar H₀ (p = 0.3849 > 0.05). Esto significa que:  
...
local_storage <- here("week-1","clase_2", "images", "p_value.png")
ggsave(local_storage, plot, width = 8, height = 5, dpi = 300)
