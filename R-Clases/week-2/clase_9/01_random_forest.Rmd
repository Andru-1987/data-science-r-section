...
```{R}
loan_data <- read_csv("R-Clases/week-2/clase_9/storage/loan_status.csv")
...
```{R}
positive_class <- "Y"  # o "1"
pred_col <- paste0(".pred_", positive_class)

rf_pred %>%
  roc_curve(truth = Loan_Status, !!sym(pred_col)) %>%
  autoplot() +
