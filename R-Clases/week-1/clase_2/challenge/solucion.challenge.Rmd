...
# 5. VISUALIZACIÓN (DESAFÍO ADICIONAL)
# =====================================

if (exists("tabla_resumen") && nrow(tabla_resumen) > 0) {
  # Gráfico de barras para distribución de clientes por país
  grafico_paises <- ggplot(tabla_resumen, aes(x = reorder(!!sym(col_pais), total_clientes), 
                                              y = total_clientes)) +
    ...
    labs(
      ...
      caption = "Datos procesados y transformados"
    ) +
    theme_minimal() +
    theme(
  ...
  grafico_edades <- ggplot(clientes_transformado, aes(x = edad_aleatoria)) +
    geom_histogram(binwidth = 5, fill = "coral", alpha = 0.7, color = "black") +
    labs(
      title = "Distribución de Edades Aleatorias",
      x = "Edad",
      y = "Frecuencia",
      caption = "Edades generadas aleatoriamente"
    ) +
    theme_minimal() +
    theme(
      plot.title = element_text(hjust = 0.5, size = 14, face = "bold")
  ...
  print(grafico_edades)
  
} else {
  cat("\nNo se pudo crear la visualización debido a problemas con las columnas de datos.")
}
