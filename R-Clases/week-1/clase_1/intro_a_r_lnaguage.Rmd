---
title: "Introducción a R: De lo Básico al mas allá"
...
## Sector donde vamos a estar enfocandonos cuando trabajemos para ds
...
# O de forma directa
df <- data.frame(numeros = c(1, 2, 3, 4, 5), letras = c("A", "B", "C", "D", "E"))
...
csv_name <- sprintf("%s.csv","write_table")
file_storage_path <- file.path( getwd(), "week-1", "clase_1", "data", csv_name)

write.table(x = df, file = file_storage_path , sep = ",")

# O usando write.csv
csv_name <- sprintf("%s.csv","write_csv")
file_storage_path <- file.path( getwd(), "week-1", "clase_1", "data", csv_name)
write.csv(df, file_storage_path, row.names = FALSE)
```
...
```{r eval=FALSE}
# Leer archivo CSV
csv_local_path <- file.path(getwd(), "week-1", "clase_1","data", "gcba_suaci_barrios.csv")
datos_importados <- utils::read.csv( csv_local_path, sep = ",", fileEncoding= "UTF-8")
...
# Lista con vectores
lista_compleja <- list(numeros = c(1, 2, 3),texto = "hola", fecha = Sys.Date(),logico = TRUE)
...
Usa el dataset `mtcars` (incluido en R) para: - Ver sus dimensiones - Calcular estadísticas descriptivas - Crear un gráfico de dispersión entre dos variables
