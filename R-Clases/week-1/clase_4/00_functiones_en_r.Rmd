...
mutation_function <- function(df){
  df <- df %>% 
  
  mutate(segmentacion = case_when(
    edad <= 29 ~ "segmento_cero",
    edad > 29 & edad <=40 ~ "segmento_uno",
    edad > 40 & edad <=70 ~ "segmento_dos",
    edad > 70 ~ "segmento_tres",
    TRUE ~ NA_character_
  )
  ) %>% 
  mutate(segmentacion = factor(x = segmentacion, levels = c("segmento_cero","segmento_uno","segmento_dos","segmento_tres"), exclude = TRUE)) %>%
  arrange(segmentacion)
...
data.frame(edad = sample(29:79, 20)) %>%
mutation_function() -> dataframe_segmentado
